...
@section('content')
    ...
    <div class="flex flex-row justify-between">
        ...
        <form action="{{ route('products.index') }}" method="GET" class="flex flex-row space-x-4 items-center">
            <select name="category" class="appearance-none inline-block px-4 py-2 shadow-sm border-gray-300 border rounded-md bg-white focus:ring ring-gray-300 focus:border-blue-300 transition ease-in-out duration-300 focus:z-10 focus:outline-none">
                <option value="tw">TW - TOOLS</option>
                <option value="tw">TW - TOOLS</option>
                <option value="tw">UNDERCONSTRUCTION</option>

            </select>
            <input type="text" name="search" class="inline-block px-4 py-2 shadow-sm border-gray-300 border rounded-md bg-white focus:ring ring-gray-300 focus:border-blue-300 transition ease-in-out duration-300 focus:z-10 focus:outline-none" placeholder="Search SKU or UPC" value="{{ request()->search }}">
            <x-button>
